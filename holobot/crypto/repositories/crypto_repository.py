...
class CryptoRepository(CryptoRepositoryInterface):
    ...
    async def get_price(self, symbol: str) -> Optional[PriceData]:
        print(f"[CryptoRepository] Getting price... {{ Symbol = {symbol} }}")
        result = await self.__cache.get_or_add(symbol, self.__load)
        print(f"[CryptoRepository] Got price. {{ Symbol = {symbol} }}")
        return result
    
    async def update_prices(self, prices: List[PriceData]):
        print("[CryptoRepository] Updating prices...]")
        ...
        print(f"[CryptoRepository] Updated prices. {{ SymbolCount = {len(prices)} }}")

    async def __load(self, symbol: str) -> Optional[PriceData]:
        print(f"[CryptoRepository] Loading price... {{ Symbol = {symbol} }}")
        async with self.__database_manager.acquire_connection() as connection:
            connection: Connection
            async with connection.transaction():
                ...
                print(f"[CryptoRepository] Loaded price. {{ Symbol = {symbol}, Exists = {result is not None} }}")
