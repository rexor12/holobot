...
class Reminders(Cog, name="Reminders"):
    ...
    async def set(self, context: Context, *, config: str):
        ...
        message = args[UNBOUND_KEY]
        if len(message) == 0 or len(message) > 120:
            await context.reply("You have to specify a message no longer than 120 characters for the reminder. Please, see the help for more information.")
            return
        if len(args["every"]) > 0:
            reminder = await self.__set_recurring_reminder(str(context.author.id), message, parse_interval(args["every"]))
        elif len(args["on"]) > 0 or len(args["at"]) > 0:
            reminder = await self.__set_single_reminder(str(context.author.id), message, parse_interval(args["at"]))
        else:
    ...
    async def __set_single_reminder(self, user_id: str, message: str, time_at: Optional[timedelta] = None) -> Reminder:
        await self.__assert_reminder_count(user_id)
        if time_at is None:
            time_at = timedelta()
