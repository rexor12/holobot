...
class ViewCommandsCommand(CommandBase):
    def __init__(self, command_registry: CommandRegistryInterface, log: LogInterface) -> None:
        super().__init__("view")
        ...
        # self.components = [
        #     ComponentRegistration("admin_commands_paginator", Paginator, self.__on_page_changed, DeferType.DEFER_MESSAGE_UPDATE)
        # ]
    ...
    async def execute(self, context: ServerChatInteractionContext, group: Optional[str] = None, subgroup: Optional[str] = None) -> CommandResponse:
        ...
        # await Pager(self.__messaging, self.__log, context, create_page_content)
        # return CommandResponse()
