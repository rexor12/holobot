...
class MuteUserCommand(ModerationCommandBase):
    ...
    async def execute(self, context: ServerChatInteractionContext, user: str, reason: str, duration: Optional[str] = None) -> CommandResponse:
        ...
        try:
            await self.__messaging.send_private_message(user_id, f"You have been muted in {context.server_id} by {context.author_id} with the reason '{reason}'. I'm sorry this happened to you.")
        except ForbiddenError:
            pass
