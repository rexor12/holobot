...
class BanUserMenuItem(ModerationMenuItemBase):
    ...
    async def execute(self, context: MenuContext, **kwargs) -> MenuItemResponse:
        ...
        try:
            await self.__user_manager.ban_user(str(context.guild_id), str(user_id), "Issued via menu item")
        ...
        await reply(context, f"<@{user_id}> has been banned.")
